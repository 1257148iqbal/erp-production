/**
 * Title: Assing Input Table Edit Form
 * Description: Assing Input Table Edit Form
 * Author: Iqbal Hossain
 * Date: 05-January-2022
 * Modified: 05-January-2022
 */

import '@custom-styles/merchandising/others/custom-table.scss';
import React, { Fragment, useEffect, useState } from 'react';
import DataTable from 'react-data-table-component';
import { ChevronDown, Eye, Maximize2, Menu, Minimize2 } from 'react-feather';
import { useDispatch, useSelector } from 'react-redux';
import { useHistory } from 'react-router-dom';
import {
  Button,
  Card,
  CardBody,
  CardHeader,
  CardTitle,
  Col,
  Collapse,
  FormGroup,
  Input,
  Label,
  Nav,
  NavItem,
  NavLink,
  Row,
  TabContent,
  Table,
  TabPane
} from 'reactstrap';
import CustomPreLoader from 'utility/custom/CustomPreLoader';
import CustomPagination from 'utility/custom/production/CustomPagination';
import { PlusIcon } from 'utility/custom/production/icons/CustomIcons';
import TableCustomerHeader from 'utility/custom/TableCustomerHeader';
import { randomIdGenerator } from 'utility/Utils';
import AssignInputTableRejectInfo from '../details/AssignInputTableRejectInfo';
import { fetchAssignInputTableByQuery } from '../store/actions';
import classes from '../style/AssignInptTableList.module.scss';
import AssignInputTableInfo from './AssignInputTable';

const AssingInputTableListPage = () => {
  const dispatch = useDispatch();
  const history = useHistory();

  //#region State
  const [active, setActive] = useState('1');
  // const [openModal, setOpenModal] = useState(false);
  const [rowsPerPage, setRowsPerPage] = useState(5);
  const [currentPage, setCurrentPage] = useState(1);
  const [searchTerm, setSearchTerm] = useState('');
  const [hasAssignInputTable, setHasAssignInputTable] = useState(false);
  const [hasAssignInputTableRejectInfo, setHasAssignInputTableRejectInfo] = useState(false);
  const [assignInputTable, setAssignInputTable] = React.useState([
    {
      id: randomIdGenerator(),
      cutPlanNo: 'CP001',
      buyer: 'Kuper',
      style: 'AV56567',
      po: 'PO 01',
      styleCategory: 'T-Shirt',
      color: 'Red',
      shipmentDate: '10-11-2021',
      sizeRange: 'S-XL',
      isOpen: false,
      details: [
        {
          id: randomIdGenerator(),
          comboInfo: '45-M-01',
          parts: 'F,B,Cuff +4 more',
          serialStart: 1,
          serialEnd: 25,
          quantity: 24,
          receivedDate: '23/11/2021',
          rejectInfo: ''
        },
        {
          id: randomIdGenerator(),
          comboInfo: '45-M-02',
          parts: 'F,B,Cuff +4 more',
          serialStart: 1,
          serialEnd: 25,
          quantity: 25,
          receivedDate: '23/11/2021',
          rejectInfo: ''
        },
        {
          id: randomIdGenerator(),
          comboInfo: '45-L-01',
          parts: 'F,B,Cuff +4 more',
          serialStart: 1,
          serialEnd: 25,
          quantity: 24,
          receivedDate: '23/11/2021',
          rejectInfo: ''
        },
        {
          id: randomIdGenerator(),
          comboInfo: '45-L-02',
          parts: 'F,B,Cuff +4 more',
          serialStart: 1,
          serialEnd: 25,
          quantity: 25,
          receivedDate: '23/11/2021',
          rejectInfo: ''
        }
      ]
    },
    {
      id: randomIdGenerator(),
      cutPlanNo: 'CP002',
      buyer: 'IFG',
      style: 'AV1873',
      po: 'PO 02',
      styleCategory: 'Shorts',
      color: 'Red',
      shipmentDate: '10-11-2021',
      sizeRange: 'L-XXL',
      isOpen: false,
      details: [
        {
          id: randomIdGenerator(),
          comboInfo: '46-M-01',
          parts: 'F,B,Cuff +4 more',
          serialStart: 1,
          serialEnd: 25,
          quantity: 25,
          receivedDate: '23/11/2021',
          rejectInfo: ''
        },
        {
          id: randomIdGenerator(),
          comboInfo: '46-M-02',
          parts: 'F,B,Cuff +4 more',
          serialStart: 1,
          serialEnd: 25,
          quantity: 25,
          receivedDate: '23/11/2021',
          rejectInfo: ''
        },
        {
          id: randomIdGenerator(),
          comboInfo: '46-L-01',
          parts: 'F,B,Cuff +4 more',
          serialStart: 1,
          serialEnd: 25,
          quantity: 25,
          receivedDate: '23/11/2021',
          rejectInfo: ''
        }
      ]
    },
    {
      id: randomIdGenerator(),
      cutPlanNo: 'CP003',
      buyer: 'RICHLU',
      style: 'M7825',
      po: 'PO 03',
      styleCategory: 'Jeans',
      color: 'Red',
      shipmentDate: '13-11-2021',
      sizeRange: 'M-2XL',
      isOpen: false,
      details: [
        {
          id: randomIdGenerator(),
          comboInfo: '47-M-01',
          parts: 'F,B,Cuff +4 more',
          serialStart: 1,
          serialEnd: 25,
          quantity: 25,
          receivedDate: '23/11/2021',
          rejectInfo: ''
        },
        {
          id: randomIdGenerator(),
          comboInfo: '47-M-02',
          parts: 'F,B,Cuff +4 more',
          serialStart: 1,
          serialEnd: 25,
          quantity: 25,
          receivedDate: '23/11/2021',
          rejectInfo: ''
        }
      ]
    }
  ]);
  const [assignInputTablePassed, setAssignInputTablePassed] = React.useState([
    {
      id: randomIdGenerator(),
      cutPlanNo: 'CP001',
      buyer: 'Kuper',
      style: 'AV56567',
      po: 'PO 01',
      styleCategory: 'T-Shirt',
      color: 'Red',
      shipmentDate: '10-11-2021',
      sizeRange: 'S-XL',
      isOpen: false,
      details: [
        {
          id: randomIdGenerator(),
          comboInfo: '45-M-01',
          parts: 'F,B,Cuff +4 more',
          serialStart: 1,
          serialEnd: 25,
          quantity: 24,
          assignedDate: '23/11/2021',
          zoneOwner: 'Md. Alamgir',
          line: 'Line 01',
          rejectInfo: ''
        },
        {
          id: randomIdGenerator(),
          comboInfo: '45-M-02',
          parts: 'F,B,Cuff +4 more',
          serialStart: 1,
          serialEnd: 25,
          quantity: 24,
          assignedDate: '23/11/2021',
          zoneOwner: 'Md. Elias',
          line: 'Line 02',
          rejectInfo: ''
        },
        {
          id: randomIdGenerator(),
          comboInfo: '45-L-01',
          parts: 'F,B,Cuff +4 more',
          serialStart: 1,
          serialEnd: 25,
          quantity: 24,
          assignedDate: '23/11/2021',
          zoneOwner: 'Md. Alamgir',
          line: 'Line 03, Line 04',
          rejectInfo: ''
        }
      ]
    },
    {
      id: randomIdGenerator(),
      cutPlanNo: 'CP002',
      buyer: 'IFG',
      style: 'AV1873',
      po: 'PO 02',
      styleCategory: 'Shorts',
      color: 'Red',
      shipmentDate: '10-11-2021',
      sizeRange: 'L-XXL',
      isOpen: false,
      details: [
        {
          id: randomIdGenerator(),
          comboInfo: '45-M-01',
          parts: 'F,B,Cuff +4 more',
          serialStart: 1,
          serialEnd: 25,
          quantity: 24,
          assignedDate: '23/11/2021',
          zoneOwner: 'Md. Alamgir',
          line: 'Line 01',
          rejectInfo: ''
        },
        {
          id: randomIdGenerator(),
          comboInfo: '45-M-02',
          parts: 'F,B,Cuff +4 more',
          serialStart: 1,
          serialEnd: 25,
          quantity: 24,
          assignedDate: '23/11/2021',
          zoneOwner: 'Md. Elias',
          line: 'Line 02',
          rejectInfo: ''
        }
      ]
    },
    {
      id: randomIdGenerator(),
      cutPlanNo: 'CP003',
      buyer: 'RICHLU',
      style: 'M7825',
      po: 'PO 03',
      styleCategory: 'Jeans',
      color: 'Red',
      shipmentDate: '13-11-2021',
      sizeRange: 'M-2XL',
      isOpen: false,
      details: [
        {
          id: randomIdGenerator(),
          comboInfo: '45-L-01',
          parts: 'F,B,Cuff +4 more',
          serialStart: 1,
          serialEnd: 25,
          quantity: 24,
          assignedDate: '23/11/2021',
          zoneOwner: 'Md. Alamgir',
          line: 'Line 03, Line 04',
          rejectInfo: ''
        }
      ]
    }
  ]);

  //#endregion

  const { total } = useSelector(({ assignInputTableReducer }) => assignInputTableReducer);

  //#region Effect
  useEffect(() => {
    dispatch(fetchAssignInputTableByQuery({ page: currentPage, rowsPerPage }));
  }, [dispatch, rowsPerPage, currentPage]);
  //#endregion

  //#region Events

  // Function for modal open (Bundle Assigned Sewing)
  const handleOpenModalBundleAssigned = () => {
    setHasAssignInputTable(!hasAssignInputTable);
  };
  // Function for modal open (Bundle Assigned Sewing)
  const handleOpenModalAssignInputRejectInfo = () => {
    setHasAssignInputTableRejectInfo(!hasAssignInputTableRejectInfo);
  };
  // Function for modal open (Bundle Assigned Sewing)

  // Function in get data on rows per page
  const handlePerPage = e => {
    const value = parseInt(e.currentTarget.value);
    setRowsPerPage(value);
    setCurrentPage(1);
  };
  const handlePageChange = ({ selected }) => {
    setCurrentPage(selected + 1);
  };

  // ** Function in get data on search query change
  const handleFilter = val => {
    setSearchTerm(val);
    dispatch(fetchAssignInputTableByQuery({ q: val, rowsPerPage, page: currentPage }));
  };

  // ** Function for toggle Assign Input Table Pending details
  const togglePendingDetails = idx => {
    const _assignInputTable = [...assignInputTable];
    const clickedItem = _assignInputTable[idx];
    clickedItem.isOpen = !clickedItem.isOpen;
    _assignInputTable[idx] = clickedItem;
    setAssignInputTable(_assignInputTable);
  };
  // ** Function for toggle Assign Input Table Passed details
  const togglePassedDetails = idx => {
    const _assignInputTablePassed = [...assignInputTablePassed];
    const clickedItem = _assignInputTablePassed[idx];
    clickedItem.isOpen = !clickedItem.isOpen;
    _assignInputTablePassed[idx] = clickedItem;
    setAssignInputTablePassed(_assignInputTablePassed);
  };

  //#endregion

  const toggle = tab => {
    if (active !== tab) {
      setActive(tab);
    }
  };

  return (
    <>
      <Card>
        <CardHeader>
          <CardTitle tag="h4">Search Filter</CardTitle>
        </CardHeader>
        <CardBody>
          <Row>
            <Col md="12">
              <Input
                id="search-item"
                className="w-100 mt-50"
                placeholder="Search"
                type="text"
                value={searchTerm}
                onChange={e => handleFilter(e.target.value)}
              />
            </Col>
          </Row>
        </CardBody>
      </Card>

      <Card>
        <Row className="rounded rounded-3 p-1">
          <CardTitle tag="h2" className="ml-2 mt-1">
            Assign Input Table
          </CardTitle>
          <Col xs={12} sm={12} md={12} lg={12} xl={12}>
            <Nav tabs>
              <NavItem>
                <NavLink
                  active={active === '1'}
                  onClick={() => {
                    toggle('1');
                  }}
                >
                  Pending
                </NavLink>
              </NavItem>
              <NavItem>
                <NavLink
                  active={active === '2'}
                  onClick={() => {
                    toggle('2');
                  }}
                >
                  Assigned
                </NavLink>
              </NavItem>
            </Nav>
            <TableCustomerHeader
              handlePerPage={handlePerPage}
              rowsPerPage={rowsPerPage}
              searchTerm={searchTerm}
            >
              <PlusIcon
                onClick={() =>
                  history.push({
                    pathname: `external-process-new`
                  })
                }
              />
            </TableCustomerHeader>
            <TabContent
              activeTab={active}
              style={{ backgroundColor: 'white', border: 'solid #dddddd 1px' }}
            >
              <TabPane tabId="1">
                <div className="border rounded rounded-3 p-1">
                  <Row>
                    <FormGroup tag={Col} xs={12} sm={12} md={12} lg={12} xl={12}>
                      <Table
                        className={classes.assignInputTable}
                        size="sm"
                        responsive
                        bordered={true}
                      >
                        <thead className={`thead-dark table-bordered ${classes.stickyTableHead}`}>
                          <tr className="text-center">
                            <th style={{ minWidth: '4px' }}>
                              <strong>#</strong>
                            </th>
                            <th>
                              <strong>Cut Plan No</strong>
                            </th>
                            <th>
                              <strong>Buyer</strong>
                            </th>
                            <th>
                              <strong>Style</strong>
                            </th>
                            <th>
                              <strong>PO</strong>
                            </th>
                            <th>
                              <strong>Style Category</strong>
                            </th>
                            <th>
                              <strong>Color</strong>
                            </th>
                            <th>
                              <strong>Shipment Date</strong>
                            </th>
                            <th>
                              <strong>Size Range</strong>
                            </th>
                          </tr>
                        </thead>
                        <tbody className="text-center">
                          {assignInputTable.map((ep, idx) => (
                            <Fragment key={ep.id + ep.id}>
                              <tr key={ep.id}>
                                <td style={{ minWidth: '4px' }}>
                                  <Button.Ripple
                                    for="collapseId"
                                    tag={Label}
                                    onClick={() => {
                                      togglePendingDetails(idx);
                                    }}
                                    className="btn-icon"
                                    color="flat-primary"
                                  >
                                    {ep.isOpen ? (
                                      <Minimize2 id="collapseId" size={15} color="#57C69D" />
                                    ) : (
                                      <Maximize2 id="collapseId" size={15} color="#7367f0" />
                                    )}
                                  </Button.Ripple>
                                </td>
                                <td>{ep.cutPlanNo}</td>
                                <td>{ep.buyer}</td>
                                <td>{ep.style}</td>
                                <td>{ep.po}</td>
                                <td>{ep.styleCategory}</td>
                                <td>{ep.color}</td>
                                <td>{ep.shipmentDate}</td>
                                <td>{ep.sizeRange}</td>
                              </tr>
                              <tr>
                                <td
                                  colSpan={10}
                                  style={{
                                    padding: '2px 10px !important',
                                    backgroundColor: '#fff'
                                  }}
                                >
                                  <Collapse isOpen={ep.isOpen}>
                                    <DataTable
                                      progressPending={!ep.details.length}
                                      progressComponent={<CustomPreLoader />}
                                      contextActions={
                                        <Button className="btn-icon " color="flat-primary">
                                          <Menu size={26} onClick={handleOpenModalBundleAssigned} />
                                        </Button>
                                      }
                                      dense
                                      subHeader={false}
                                      highlightOnHover
                                      selectableRows
                                      responsive
                                      paginationServer
                                      expandOnRowClicked
                                      persistTableHead
                                      columns={[
                                        {
                                          name: 'Combo Info',
                                          minWidth: '170px',
                                          selector: 'comboInfo',
                                          sortable: true,
                                          cell: row => row.comboInfo
                                        },
                                        {
                                          name: 'Parts',
                                          minWidth: '150px',
                                          selector: 'parts',
                                          sortable: true,
                                          cell: row => row.parts
                                        },
                                        {
                                          name: 'Serial Start',
                                          minWidth: '150px',
                                          selector: 'serialStart',
                                          sortable: true,
                                          cell: row => row.serialStart
                                        },
                                        {
                                          name: 'Serial End',
                                          minWidth: '150px',
                                          selector: 'serialEnd',
                                          sortable: true,
                                          cell: row => row.serialEnd
                                        },
                                        {
                                          name: 'Quantity',
                                          minWidth: '150px',
                                          selector: 'quantity',
                                          sortable: true,
                                          cell: row => row.quantity
                                        },
                                        {
                                          name: 'Received Date',
                                          minWidth: '150px',
                                          selector: 'receivedDate',
                                          sortable: true,
                                          cell: row => row.receivedDate
                                        },
                                        {
                                          name: 'Received Date',
                                          minWidth: '150px',
                                          selector: 'receivedDate',
                                          sortable: true,
                                          cell: () => (
                                            <Eye
                                              onClick={handleOpenModalAssignInputRejectInfo}
                                              className="cursor-pointer"
                                            ></Eye>
                                          )
                                        }
                                      ]}
                                      sortIcon={<ChevronDown />}
                                      className="react-dataTable react-table-height"
                                      data={ep.details}
                                    />
                                  </Collapse>
                                </td>
                              </tr>
                            </Fragment>
                          ))}
                        </tbody>
                      </Table>
                    </FormGroup>
                  </Row>
                </div>
              </TabPane>
              <TabPane tabId="2">
                <div className="border rounded rounded-3 p-1">
                  <Row>
                    <FormGroup tag={Col} xs={12} sm={12} md={12} lg={12} xl={12}>
                      <Table
                        className={classes.assignInputTable}
                        size="sm"
                        responsive
                        bordered={true}
                      >
                        <thead className={`thead-dark table-bordered ${classes.stickyTableHead}`}>
                          <tr className="text-center">
                            <th style={{ minWidth: '4px' }}>
                              <strong>#</strong>
                            </th>
                            <th>
                              <strong>Cut Plan No</strong>
                            </th>
                            <th>
                              <strong>Buyer</strong>
                            </th>
                            <th>
                              <strong>Style</strong>
                            </th>
                            <th>
                              <strong>PO</strong>
                            </th>
                            <th>
                              <strong>Style Category</strong>
                            </th>
                            <th>
                              <strong>Color</strong>
                            </th>
                            <th>
                              <strong>Shipment Date</strong>
                            </th>
                            <th>
                              <strong>Size Range</strong>
                            </th>
                          </tr>
                        </thead>
                        <tbody className="text-center">
                          {assignInputTablePassed.map((ep, idx) => (
                            <Fragment key={ep.id + ep.id}>
                              <tr key={ep.id}>
                                <td style={{ minWidth: '4px' }}>
                                  <Button.Ripple
                                    for="collapseId"
                                    tag={Label}
                                    onClick={() => {
                                      togglePassedDetails(idx);
                                    }}
                                    className="btn-icon"
                                    color="flat-primary"
                                  >
                                    {ep.isOpen ? (
                                      <Minimize2 id="collapseId" size={15} color="#57C69D" />
                                    ) : (
                                      <Maximize2 id="collapseId" size={15} color="#7367f0" />
                                    )}
                                  </Button.Ripple>
                                </td>
                                <td>{ep.cutPlanNo}</td>
                                <td>{ep.buyer}</td>
                                <td>{ep.style}</td>
                                <td>{ep.po}</td>
                                <td>{ep.styleCategory}</td>
                                <td>{ep.color}</td>
                                <td>{ep.shipmentDate}</td>
                                <td>{ep.sizeRange}</td>
                              </tr>
                              <tr>
                                <td
                                  colSpan={10}
                                  style={{
                                    padding: '2px 10px !important',
                                    backgroundColor: '#fff'
                                  }}
                                >
                                  <Collapse isOpen={ep.isOpen}>
                                    <Table className={classes.childTable}>
                                      <thead className="thead-light table-bordered">
                                        <tr>
                                          <th></th>
                                          <th>Combo Info</th>
                                          <th>Parts</th>
                                          <th>Serial Start</th>
                                          <th>Serial End</th>
                                          <th>Quantity</th>
                                          <th>Assigned Date</th>
                                          <th>Zone Owner</th>
                                          <th>Line</th>
                                          <th>Reject Info</th>
                                        </tr>
                                      </thead>
                                      <tbody>
                                        {ep.details.map(bundle => (
                                          <tr key={bundle.id}>
                                            <td></td>
                                            <td>{bundle.comboInfo}</td>
                                            <td>{bundle.parts}</td>
                                            <td>{bundle.serialStart}</td>
                                            <td>{bundle.serialEnd}</td>
                                            <td>{bundle.quantity}</td>
                                            <td>{bundle.assignedDate}</td>
                                            <td>{bundle.zoneOwner}</td>
                                            <td>{bundle.line}</td>
                                            <td>
                                              <Eye
                                                onClick={handleOpenModalAssignInputRejectInfo}
                                                className="cursor-pointer"
                                              ></Eye>
                                            </td>
                                          </tr>
                                        ))}
                                      </tbody>
                                    </Table>
                                  </Collapse>
                                </td>
                              </tr>
                            </Fragment>
                          ))}
                        </tbody>
                      </Table>
                    </FormGroup>
                  </Row>
                </div>
              </TabPane>
            </TabContent>
          </Col>
        </Row>
        <CustomPagination
          count={Number(Math.ceil(total / rowsPerPage))}
          currentPage={currentPage}
          onPageChange={handlePageChange}
        />
      </Card>
      {hasAssignInputTable && (
        <AssignInputTableInfo
          openModal={hasAssignInputTable}
          setOpenModal={setHasAssignInputTable}
        />
      )}
      {hasAssignInputTableRejectInfo && (
        <AssignInputTableRejectInfo
          openModal={hasAssignInputTableRejectInfo}
          setOpenModal={setHasAssignInputTableRejectInfo}
        />
      )}
    </>
  );
};

export default AssingInputTableListPage;

/** Change Log
 * 26-Jan-2022 (Iqbal): Assign Input Table List Page and Tab With data render from mock and Onchange events
 */
